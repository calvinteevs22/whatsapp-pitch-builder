...
import { validateUtilityCompliance, sanitizeUtilityMessages } from "../shared/utilityComplianceRules";
  ...
  ai: router({
    generateFlow: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // POST-PROCESS: Utility compliance — strip promotional content from utility flows
        if (input.messageType === "utility") {
          const complianceCheck = validateUtilityCompliance(parsed.messages, "utility");
          if (!complianceCheck.isCompliant) {
            console.log(`[AI Gen] Utility compliance: ${complianceCheck.violationCount} violation(s) detected, auto-sanitizing...`);
            const sanitized = sanitizeUtilityMessages(parsed.messages);
            if (sanitized.fixCount > 0) {
              parsed.messages = sanitized.messages;
              console.log(`[AI Gen] Utility compliance: auto-fixed ${sanitized.fixCount} issue(s): ${sanitized.fixes.join("; ")}`);
            }
            // Re-validate after sanitization
            const recheck = validateUtilityCompliance(parsed.messages, "utility");
            if (!recheck.isCompliant) {
              console.warn(`[AI Gen] Utility compliance: ${recheck.violationCount} violation(s) remain after auto-fix — flagging for review`);
            }
          } else {
            console.log(`[AI Gen] Utility compliance: passed — no promotional content detected`);
          }
        }
...
Return JSON with: { "headline": "...", "primaryText": "...", "ctaText": "..." }`;
        ...
        // POST-PROCESS: Final utility compliance check (for metadata return)
        let complianceMetadata: { isCompliant: boolean; violationCount: number; summary: string; violations: any[] } | undefined;
        if (input.messageType === "utility") {
          const finalCheck = validateUtilityCompliance(parsed.messages, "utility");
          complianceMetadata = {
            isCompliant: finalCheck.isCompliant,
            violationCount: finalCheck.violationCount,
            summary: finalCheck.summary,
            violations: finalCheck.violations,
          };
          console.log(`[AI Gen] Utility compliance final: ${finalCheck.summary}`);
        }

        // Images are already generated — no background polling needed
        return { ...parsed, imagesGenerating: false, savedMessages: parsed.savedMessages || null, adCreative: generatedAdCreative, compliance: complianceMetadata };
      }),
...
function buildSystemPrompt(messageType: string, industry?: string | null, language?: string): string {
...
function buildUserPrompt(input: { prompt: string; businessName?: string; businessUrl?: string; industry?: string | null; subVertical?: string; messageType: string }): string {
  ...
  if (input.subVertical) {
    const SUB_VERTICAL_CONTEXT: Record<string, Record<string, string>> = {
      "E-Commerce": {
        "Fashion & Apparel": "This is a FASHION & APPAREL e-commerce business (clothing, shoes, accessories). Use fashion-specific product names (dresses, sneakers, handbags), realistic fashion pricing (e.g., $1,999 for a top, $4,999 for a jacket), fashion imagery (lookbooks, outfit flatlays, model shots), and fashion terminology (collection, lookbook, style, fit, fabric). The business name and products should clearly be a fashion brand.",
        "Grocery & Food Delivery": "This is a GROCERY & FOOD DELIVERY e-commerce business (fresh produce, pantry staples, meal kits). Use grocery-specific products (fruits, vegetables, dairy, snacks), realistic grocery pricing (e.g., $480 for a fruit basket, $1,850 for a weekly basket), food imagery (fresh produce, grocery bags, meal ingredients), and grocery terminology (fresh, organic, delivery slot, basket, refill). The business should clearly be an online grocery store.",
        "Electronics & Gadgets": "This is an ELECTRONICS & GADGETS e-commerce business (smartphones, laptops, accessories). Use electronics-specific products (phones, earbuds, laptops, smartwatches), realistic tech pricing (e.g., $79,900 for a flagship phone, $2,499 for earbuds), tech imagery (product shots on clean backgrounds, unboxing, spec sheets), and tech terminology (specs, warranty, trade-in, upgrade, storage). The business should clearly be an electronics retailer.",
        "Beauty & Cosmetics": "This is a BEAUTY & COSMETICS e-commerce business (skincare, makeup, haircare). Use beauty-specific products (serums, foundations, lipsticks, moisturizers), realistic beauty pricing (e.g., $1,299 for a serum, $899 for a moisturizer), beauty imagery (product bottles on marble, swatches, skin close-ups), and beauty terminology (routine, shade, skin type, ingredients, SPF). The business should clearly be a beauty/cosmetics brand.",
        "Home & Furniture": "This is a HOME & FURNITURE e-commerce business (furniture, decor, home essentials). Use home-specific products (sofas, dining tables, lamps, rugs, cushions), realistic furniture pricing (e.g., $45,999 for a sofa set, $1,999 for a throw), home imagery (styled room shots, furniture in context, decor arrangements), and home terminology (room, assembly, delivery, dimensions, material). The business should clearly be a home/furniture store.",
        "Health & Pharmacy": "This is a HEALTH & PHARMACY e-commerce business (medicines, supplements, health devices). Use health-specific products (prescription medicines, vitamins, supplements, BP monitors), realistic pharmacy pricing (e.g., $245 for a prescription refill, $499 for supplements), health imagery (medicine bottles, supplement packs, health devices), and health terminology (prescription, dosage, refill, wellness, adherence). The business should clearly be an online pharmacy.",
        "Jewelry & Luxury": "This is a JEWELRY & LUXURY e-commerce business (fine jewelry, watches, luxury accessories). Use jewelry-specific products (diamond rings, gold necklaces, pearl earrings, luxury watches), realistic luxury pricing (e.g., $18,500 for diamond studs, $45,000 for a custom ring), luxury imagery (jewelry on velvet, close-up gemstone shots, elegant gift boxes), and luxury terminology (carat, certification, engraving, bespoke, 18K gold). The business should clearly be a jewelry/luxury brand.",
        "Sports & Outdoors": "This is a SPORTS & OUTDOORS e-commerce business (fitness equipment, sportswear, outdoor gear). Use sports-specific products (dumbbells, running shoes, yoga mats, cycling gear), realistic sports pricing (e.g., $4,999 for a dumbbell set, $1,299 for resistance bands), sports imagery (gym equipment, active lifestyle shots, outdoor adventures), and sports terminology (fitness goal, training, gear, performance, endurance). The business should clearly be a sports/fitness retailer.",
      },
      "Real Estate": {
